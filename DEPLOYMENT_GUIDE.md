...
**問題**：部署後網站顯示 "CI 環境：跳過 Firebase 初始化" 但仍嘗試連接 `fake-project-id`

**原因**：GitHub Actions 工作流程錯誤地同時覆寫了 `environment.ts` 和 `environment.prod.ts`

**修正**：

1. 移除 GitHub Actions 中覆寫 `environment.ts` 的步驟
2. 僅在生產建置時替換 `environment.prod.ts`
3. 修正 `setup:ci-env` 腳本，避免覆寫生產環境配置

**結果**：

- CI 環境中：使用 `environment.ci.ts`（跳過 Firebase）
- 生產環境中：使用 `environment.prod.ts`（從 GitHub Secrets 生成）
- Angular 的 fileReplacements 確保生產建置使用正確的環境文件
